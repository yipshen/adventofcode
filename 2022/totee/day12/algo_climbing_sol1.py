...
def initDijkstra(matrice, cellDebut):
    infini = 10000
    maxRow, maxCol = np.shape(matrice)
    for i in range(0,maxRow):
        for j in range(0,maxCol):
...
def initAstar(matrice, cellDebut):
    infini = 10000
    maxRow, maxCol = np.shape(matrice)
    for i in range(0,maxRow):
        for j in range(0,maxCol):
...
def dijkstraFX(cellDebut, cellFin, spritesfield, screen):
    ...
    while stackPriority:
        ...
        if currentCell == cellFin:
            ...
            logger.debug(f"dijkstraFX - pathLengh : {carte[currentCell].getDistance() } caseVisited:{nbCaseVisited}")
            break
...
def astar(cellDebut, cellFin, spritesfield, screen ):
    ...
    while stackPriority:
        ...
        if currentCell == cellFin:
            ...
            logger.debug(f"astar -  pathLengh : {carte[currentCell].getDistance()} caseVisited:{nbCaseVisited}")
            break
    currentCell = carte[currentCell].getParent()
        ...
        for event in event_list:  # On parcours la liste de tous les événements reçus
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_a:
                    listCellFollowd = []
                    # exploreRandomMap(cellStart, cellEnd, listCellFollowd, spritesfield, screen)
                    astar(cellStart,cellEnd,spritesfield, screen)
            if event.type == pygame.KEYDOWN:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_m:
                    resetMap(carte)
                    dijkstra(cellStart, cellEnd, spritesfield, screen)
            if event.type == pygame.KEYDOWN:
