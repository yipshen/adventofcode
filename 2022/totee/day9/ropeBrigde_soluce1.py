...
def updateMatrice(matrice, headCur, tailCur, headNw, tailNw):
    ...
    logger.debug(f"updateMatrice : headCurPos: {headCurPos} headNewPos: {headNewPos}  tailCurPos: {tailCurPos} tailNewPos: {tailNewPos} ")
...
def main():
    ...
    for cpt,move in enumerate(listMoves):
        ...
        HMoves, Tmoves = moveHeaderNTail(m, header, tail, start, move)
